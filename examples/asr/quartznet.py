...
def construct_name(name, lr, batch_size, num_epochs, wd, optimizer, kernel_size_factor):
    return "{0}-lr_{1}-bs_{2}-e_{3}-wd_{4}-opt_{5}-kfactor{6}".format(name, lr, batch_size, num_epochs, wd, optimizer,
                                                                      kernel_size_factor)
...
def create_all_dags(args, neural_factory):
    ...
    for idx in range(len(quartz_params["JasperEncoder"]["jasper"]) - 1):
        quartz_params["JasperEncoder"]["jasper"][idx]["kernel_size_factor"] = args.kernel_size_factor
