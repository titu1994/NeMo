...
class NoamAnnealingV2(_LRScheduler):
    def __init__(
        self,
        optimizer,
        *,
    ...
    def _noam_annealing(self, initial_lr, step):
        if step <= self.warmup_steps:
            mult = step * (self.warmup_steps ** (-(1.0 + self._decay)))
        else:
            mult = step ** (-self._decay)
        # mult = self._normalize * min(step ** (-self._decay), step * (self.warmup_steps ** (-(1.0 + self._decay))))
        mult = self._normalize * mult
        out_lr = initial_lr * mult
        if step > self.warmup_steps:
            out_lr = max(out_lr, self.min_lr)
        return out_lr
...
AVAILABLE_SCHEDULERS = {
    ...
    'NoamAnnealingV2': NoamAnnealingV2,
